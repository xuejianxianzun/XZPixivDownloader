...
if (window.location.href.indexOf("whitecube") > -1) {
	...
	var pageType = 0, //页面类型的分类
		maxGetNum = -1, //向下获取多少个页面。0位仅获取当前页，-1为不限制
		ajax1Bingfa = 5, //抓取页面（而非下载图片）时的并发数
		ajax1Jiange = 100, //抓取页面的并发请求每个间隔多少毫秒
		ajax1No = 0, //统计有几个并发线程完成所有请求
		allPageUrl = [], //储存列表的url
		doc0_jq, //将获取的html源码解析为DOM并转换为jq对象
		doc1_jq, //将获取的html源码解析为DOM并转换为jq对象
		pNum, //多P作品的P数
		imgUrlList = [], //储存图片最终url的数组
		imgNum = 0, //imgUrlList的下标
		nowhtml, //输出内容时使用
		ajaxDocIsEnd = true, //使用ajax加载页面并解析页面元素的任务是否执行完毕
		checkURLIsEnd = true, //检查网址正确性的函数是否执行完毕
		testURL_PHPFile = "http://saber.love/demo/testURL.php";
	...
	// 对于多图作品，判断url是否可用（因为未抓取到最深层，所以抓取到的多p作品后缀名都是jpg，这里判断jpg是否正确）
	function checkURL(url, callback, type, pNum) {
		checkURLIsEnd = false;
		$.ajax({
			url: testURL_PHPFile + "?url=" + url,
			type: "get",
			async: true,
			cache: false,
			dataType: "text",
			success: function(data) {
				if (data == 200) {
					callback(true, type, url, pNum);
				} else {
					callback(false, type, url, pNum);
				}
			}
		});
	}

	// checkURL的回调函数，处理后缀名并添加进数组
	var checkURLResult = function(result, type, url, pNum) {
		//保存变量避免被后执行的代码覆盖
		...
		if (nowResult === true) {
			// 结果为true
			if (nowType == 1) { //多图作品
				var urlSplit = nowUrl.split("_p");
				for (var i = pNo - 1; i >= 0; i--) {
					imgUrlList[imgNum] = urlSplit[0] + "_p" + i + urlSplit[1].split("0")[1];
					output();
				}
			} else if (nowType == 2) { //big作品
				imgUrlList[imgNum] = nowUrl;
				output();
			}
		} else if (nowResult === false) {
			// 结果为false 说明之前抓取到的图片url是错的
			if (nowType == 1) { //多图作品
				// console.log("错误的url:" + nowUrl);
				nowUrl = nowUrl.replace("jpg", "png");
				var urlSplit = nowUrl.split("_p");
				for (var i = pNo - 1; i >= 0; i--) {
					imgUrlList[imgNum] = (urlSplit[0] + "_p" + i + urlSplit[1].split("0")[1]);
					output();
				...
				imgUrlList[imgNum] = nowUrl.replace("jpg", "png");
				output();
			}
		}
		// 检查网址并添加到数组的动作执行完毕
	...
	if (window.location.href.indexOf("illust_id") > -1) { //1.on_illust_list，作品页内页
		pageType = 1;
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (!!doc1_jq.find(".original-image")[0]) { //如果是单图
						imgUrlList[imgNum] = doc1_jq.find(".original-image").attr("data-src");
						output();
						...
						if (!!doc1_jq.find(".full-screen._ui-tooltip")[0]) { //如果是动图 DOMParser之后无法解析（找到）canvas标签，不然判断更简单
							imgUrlList[imgNum] = doc1_jq.find("#wrapper script").eq(0).text().split("\"src\":\"")[2].split("\",\"mime_type")[0].replace(/\\/g, ""); //DOMParser之后js不会执行，所以无法用p站自己的动图变量来获取动图网址了
							output();
						} else {
							if (doc1_jq.find(".works_display a").eq(0).attr("href").indexOf("mode=manga") > -1) { //如果有mode=manga，是多图
								pNum = Number(doc1_jq.find("ul.meta li").eq(1).text().split(" ")[1].split("P")[0]); //P数
								var thumbHref = doc1_jq.find(".works_display img").attr("src"); //缩略图url
								var bigImgHref = thumbHref.replace("600x600", "1200x1200"); //第一张大图的url
								var imgUrlSplit = bigImgHref.split("p0");
								var tempUrl = (imgUrlSplit[0] + "p0" + imgUrlSplit[1]).replace("c/1200x1200/img-master", "img-original").replace("_master1200", ""); //用于测试的url，为第一张图片的url，扩展名是jpg
								checkURL(tempUrl, checkURLResult, 1, pNum);
							} else if (doc1_jq.find(".works_display a").eq(0).attr("href").indexOf("mode=big") > -1) { //对于mode=big的情况，不再进行跳转。因为没有refer会导致big页跳转回作品页，那样会抓取错误。
								var trueurl = doc1_jq.find(".bookmark_modal_thumbnail").attr("data-src");
								trueurl = trueurl.replace("c/150x150/img-master", "img-original");
								trueurl = trueurl.replace("_master1200", "");
								checkURL(trueurl, checkURLResult, 2);
							}
						}
					}
					ajaxDocIsEnd = true;
					...
					if (!!doc1_jq.find(".after a")[0] && maxGetNum === -1 || !!doc1_jq.find(".after a")[0] && maxGetNum > 0) { //如果存在下一个作品，则
						ajaxfordoc1("http://www.pixiv.net/" + doc1_jq.find(".after a").eq(0).attr("href"));
					} else { //没有剩余作品
						outputUrls(imgUrlList);
					}
				},
		...
		(function() {
			...
			$(".action")[0].insertBefore(startBotton, $(".bookmark-container")[0]);
			$(startBotton).text("从本页开始下载图片");
			startBotton.style.cssText = "background: #DE2D35;border-radius: 3px;color: #fff;text-align: center;padding: 2px 5px;cursor: pointer;position: absolute;top: -1px;left: 480px;";
			startBotton.addEventListener("click", function() {
				...
				nowhtml = $(outputInfo).html();
				//开始获取图片
				ajaxfordoc1(window.location.href);
			}, false);
		})();
	} else if ((window.location.href.indexOf("member_illust.php?id=") > -1 || window.location.href.indexOf("&id=") > -1) && (window.location.href.indexOf("&tag") == -1 && window.location.href.indexOf("?tag") == -1)) { //2.on_illust_list
		pageType = 2;
		...
		var baseUrl = window.location.href.split("&p=")[0] + "&p=";
					...
					if (!!doc1_jq.find(".original-image")[0] && !tagIsFound) { //如果是单图
						...
						if (!!doc1_jq.find(".full-screen._ui-tooltip")[0]) { //如果是动图 DOMParser之后无法解析（找到）canvas标签，不然判断更简单
							...
							if (doc1_jq.find(".works_display a").eq(0).attr("href").indexOf("mode=manga") > -1) { //如果有mode=manga，是多图
								...
								var tempUrl = (imgUrlSplit[0] + "p0" + imgUrlSplit[1]).replace("c/1200x1200/img-master", "img-original").replace("_master1200", ""); //用于测试的url，为第一张图片的url，扩展名是jpg
								checkURL(tempUrl, checkURLResult, 1, pNum);
							} else if (doc1_jq.find(".works_display a").eq(0).attr("href").indexOf("mode=big") > -1) { //对于mode=big的情况，不再进行跳转
								var trueurl = doc1_jq.find(".bookmark_modal_thumbnail").attr("data-src");
								trueurl = trueurl.replace("c/150x150/img-master", "img-original");
								trueurl = trueurl.replace("_master1200", "");
								checkURL(trueurl, checkURLResult, 2);
	...
	} else if ((window.location.href.indexOf("bookmark.php") > -1 && window.location.href.indexOf("tag=") > -1) || (window.location.href.indexOf("member_illust.php?id=") > -1 && window.location.href.indexOf("&tag=") > -1)) { //3.on_tagpage
		pageType = 3;
	...
	} else if (window.location.href.indexOf("bookmark.php") > -1 && window.location.href.indexOf("tag=") == -1) { //4.on_bookmark
		pageType = 4;
		...
		var baseUrl = window.location.href.split("?rest=show&p=")[0] + "?rest=show&p=";

		if (!!$(".page-list .current")[0]) { //如果显示有页码
			var startPageNo = Number($(".page-list .current").eq(0).text()); //最开始时的页码
			var baseUrl = "http://www.pixiv.net/bookmark.php" + $(".page-list").eq(0).find("a").eq(0).attr("href").split("&p=")[0] + "&p="; //这个基础url要根据排序方式变化，所以从页码中取值
		} else { //否则认为只有1页
			var startPageNo = 1;
	...
	} else if (window.location.href.indexOf("search.php?") > -1) { //5.on_tagsearch
		pageType = 5;
		...
		if (window.location.href !== "http://www.pixiv.net/ranking.php") {
