...
! function() {
	if (window.location.href.indexOf("whitecube") > -1) {
		...
		// 对于多图作品，判断url里的后缀名是否正确（因为未抓取到最深层，所以抓取到的多p作品后缀名都是jpg，需要判断到底是jpg还是png）
		function checkURL(url, type, pNum) {
			checkURLIsEnd = false;
			$.ajax({
				...
				success: function(data) {
					if (data == 200) {
						...
						if (type == 1) { //多图作品
							...
							imgUrlList[imgNum] = url.replace("jpg", "png");
							output();
						}
					}
					// 检查网址并添加到数组的动作执行完毕
						...
						if (!!doc1_jq.find(".original-image")[0]) { //如果是单图
							...
							if (!!doc1_jq.find(".full-screen._ui-tooltip")[0]) { //如果是动图 DOMParser之后无法解析（找到）canvas标签，不然判断更简单
								...
								if (doc1_jq.find(".works_display a").eq(0).attr("href").indexOf("mode=manga") > -1) { //如果有mode=manga，是多图
									var pNum = Number(doc1_jq.find("ul.meta li").eq(1).text().split(" ")[1].split("P")[0]); //P数
