...
if (user_lang === "简体中文" || user_lang === "繁體中文") { // 设置语言为中文
	lang_type = 0;
	console.log("lang_type = 0");
} else if (user_lang === "日本語") { // 设置语言为日文
	lang_type = 1;
	console.log("lang_type = 1");
} else { // 设置语言为英文
	lang_type = 2;
	console.log("lang_type = 2");
}
var xz_lang = { // 储存语言配置。在属性名前面加上下划线，和文本内容做出区别。{}表示需要进行替换的部分
	"_过滤作品类型的按钮": [
		"排除指定类型的作品",
		"タイプでフィルタリングする",
		"Filter by illustration type"
	],
	"_过滤作品类型的按钮_title": [
		"在下载前，您可以设置想要排除的作品类型。",
...
function xzlt(name, vals) { // xianzun_lang_translate 翻译
			...
			if (page_type === 5) { // tag搜索页
				...
				for (var i = 0; i < allPicArea.length; i++) {
					...
					if (shoucang >= want_favorite_number) {
						...
						$(window.top.document).find(tag_search_lv1_selector)[0].appendChild(allPicArea[i]);
					}
				}
...
function addBtnsAreaCtrl() {
	...
	xianzun_btns_wrap.outerHTML =
		...
		'<div class="xz_btns_ctr" data-show="0"><span class="xianzun_arrow_left"></span><span>展开下载按钮</span></div>' +
	...
	styleE.innerHTML +=
		...
		'.xz_btns_ctr{width: 16px;padding: 11px 8px;cursor: pointer;display: inline-block;vertical-align: top;font-size: 14px;text-align: center;background: #34b0e0;color: #fff;border-radius: 5px;transition: background .3s;}' +
...
function addOutputWarp() {
	...
	outputWrap.outerHTML =
		...
		'<div class="pauseDownload" style="background:#d29203;">' + xzlt("_下载按钮2") + '</div>' +
		'<div class="stopDownload" style="background:#DE0000;">' + xzlt("_下载按钮3") + '</div>' +
...
if (loc_url.indexOf("illust_id") > -1 && loc_url.indexOf("mode=manga") == -1 && loc_url.indexOf("bookmark_detail") == -1 && loc_url.indexOf("bookmark_add") == -1) { //1.on_illust_list，作品页内页
	...
	*/
	...
	if ($("#js-mount-point-search-result-list").length > 0) { // tag搜索页新版
		...
		tag_search_list_selector = "#js-mount-point-search-result-list ._7IVJuWZ";
	...
	function listSort() {
		...
		for (var i = 0; i < imgList.length; i++) {
			$(tag_search_lv1_selector).append(imgList[i].e);
		}
		//处理tag列表页预览图的懒加载
	...
	setFilterTag_Need(3);
	setFilterTag_notNeed(4);
} else if (window.location.pathname === "/discover") { //11.discover 发现
	...
	(function() {
		...
		$(clearMultiple).text(xzlt("_清除多图作品"));
		$(clearMultiple).attr("title", xzlt("_清除多图作品_title"));
		setButtonStyle(clearMultiple, 4, "#DE0101");
		clearMultiple.addEventListener("click", function() {
			var allPicArea = $(tag_search_list_selector);
			for (var i = 0; i < allPicArea.length; i++) {
				if (!!allPicArea.eq(i).find(tag_search_multiple_selector)[0]) {
	...
	(function() {
		...
		$(clearUgoku).text(xzlt("_清除动图作品"));
		$(clearUgoku).attr("title", xzlt("_清除动图作品_title"));
		setButtonStyle(clearUgoku, 5, "#DE0101");
		clearUgoku.addEventListener("click", function() {
			var allPicArea = $(tag_search_list_selector);
			for (var i = 0; i < allPicArea.length; i++) {
				if (!!allPicArea.eq(i).find(tag_search_gif_selector)[0]) {
	...
	(function() {
		...
		$(deleteBotton).attr("title", xzlt("_手动删除作品_title"));
		$(deleteBotton).attr("data_del", "0");
		setButtonStyle(deleteBotton, 6, "#DE0000");
		$("#deleteBotton").bind("click", function() {
			$(tag_search_list_selector).bind("click", function() {
				if ($("#deleteBotton").attr("data_del") === "1") {
					this.remove();
	...
	(function() {
		...
		$(clearBotton).text(xzlt("_清空作品列表"));
		$(clearBotton).attr("title", xzlt("_清空作品列表_title"));
		setButtonStyle(clearBotton, 7, "#DE0000");
		clearBotton.addEventListener("click", function() {
			$(tag_search_list_selector).remove();
		}, false);
	})();
