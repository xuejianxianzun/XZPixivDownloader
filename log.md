...
在作品页内下载还不行，先在 _无权访问1 里添加提示。下个版本再修改为不依赖jQuery。

此外，现在tag里有一些附带了英文tag，如id=68322571。后续需要检查一下旧版获取tag的代码。
nowAllTagE =

另外，新版直接显示收藏数了，等以后统一是新版了，可以更新到命名规则里。

新版从json信息获取info的方法也可以看能不能用在旧版上。（但是以后旧版可能全部会变成新版，这样的话就没必要改了）
